...
import { ArrowRight, X, Lock, Loader2, CalendarClock } from 'lucide-react'
...
export function MoveLeadModal({ lead, targetStage, labels, onConfirm, onClose, canConfirmPago, userRole }: {
  ...
  const blockedReunionNoSchedule = isAgendadora && !lead.has_reunion_scheduled
      ...
      <div className="w-full max-w-md rounded-2xl overflow-hidden"
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {blockedReunionNoSchedule && !blockedAdvanceFromReunion && (
            <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-xl"
              style={{ background: 'rgba(67,97,238,0.07)', border: '1px solid rgba(67,97,238,0.25)' }}>
              <CalendarClock size={13} style={{ color: '#4361ee', flexShrink: 0, marginTop: 1 }} />
              <div>
                <p className="text-xs font-bold" style={{ color: '#4361ee' }}>Reunión no agendada</p>
                <p className="text-[10px] mt-0.5" style={{ color: 'rgba(67,97,238,0.80)' }}>
