...
const TEXT      = '#1a2035'
const TEXT_MUTED = 'rgba(26,32,53,0.45)'
const CARD_BG   = '#ffffff'
const COL_BG    = 'rgba(26,32,53,0.03)'
const DIVIDER   = 'rgba(26,32,53,0.08)'
  ...
  return (
    <button onClick={() => onSelect(lead)} className="w-full text-left block">
      <div className="rounded-2xl p-4 transition-all duration-150"
        style={{ background: CARD_BG, border: `1px solid ${stage.border}`, boxShadow: '0 1px 4px rgba(26,32,53,0.06)' }}
        onMouseEnter={e => {
        ...
        onMouseLeave={e => {
          const el = e.currentTarget as HTMLElement
          el.style.boxShadow = '0 1px 4px rgba(26,32,53,0.06)'
...
function KanbanColumn({ stage, leads, onSelect }: { stage: typeof STAGES[0]; leads: CobradorLead[]; onSelect: (l: CobradorLead) => void }) {
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0 rounded-2xl overflow-hidden"
      style={{ border: `1px solid ${DIVIDER}`, background: COL_BG }}>

      {/* Header */}
      <div className="flex-shrink-0 px-4 py-3.5" style={{ borderBottom: `1px solid ${DIVIDER}`, background: '#ffffff' }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <span className="w-2.5 h-2.5 rounded-full" style={{ background: stage.color }} />
            <span className="text-sm font-black" style={{ color: TEXT }}>{stage.label}</span>
          </div>
        ...
        {totalDeuda > 0 && (
          <p className="text-xs font-bold mt-1.5 ml-5" style={{ color: stage.color }}>{fmt(totalDeuda)}</p>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ minHeight: 0 }}>
        {leads.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 gap-3" style={{ opacity: 0.35 }}>
            <div className="w-12 h-12 rounded-full border-2 flex items-center justify-center" style={{ borderColor: stage.color }}>
              <Users size={18} style={{ color: stage.color }} />
            </div>
            <p className="text-xs font-semibold" style={{ color: stage.color }}>Sin clientes</p>
          </div>
        ) : (
...
function DetailDrawer({ lead, onUpdate, onClose }: { lead: CobradorLead; onUpdate: (l: CobradorLead) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex" style={{ background: 'rgba(26,32,53,0.45)' }} onClick={onClose}>
      <div className="ml-auto h-full w-full max-w-sm flex flex-col overflow-hidden"
        style={{ background: '#ffffff', boxShadow: '-12px 0 40px rgba(26,32,53,0.15)', borderLeft: `1px solid ${DIVIDER}` }}
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-4 py-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleSaveMonto} disabled={savingMonto}
                ...
                style={{ background: 'rgba(67,97,238,0.10)', color: '#4361ee', border: '1.5px solid rgba(67,97,238,0.22)' }}>
                {savingMonto ? '...' : 'OK'}
              </button>
            </div>
            {parseFloat(montoPagado) > 0 && (
              <p className="mt-1.5 text-sm font-black" style={{ color: '#4361ee' }}>
          ...
          <div className="px-4 py-3">
            ...
            <button onClick={handleSaveNotes}
              ...
              style={{ background: 'rgba(67,97,238,0.08)', color: '#4361ee', border: '1.5px solid rgba(67,97,238,0.20)', opacity: notes === (lead.notes ?? '') ? 0.4 : 1 }}>
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="w-6 h-6 border-2 rounded-full animate-spin" style={{ borderColor: 'rgba(26,32,53,0.10)', borderTopColor: '#4361ee' }} />
      </div>
    )
  }
