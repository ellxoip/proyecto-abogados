...
import { generarAccesoPagaCuotasAction } from "@/app/cuotas/[contratoId]/actions";
...
export function RegistrarPagoButton({ contratoId }: { contratoId: number }) {
  ...
  function handleClick() {
    ...
    startTransition(async () => {
      try {
        const result = await generarAccesoPagaCuotasAction(contratoId);
        setInfo(result);
      } catch (e) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        {pending ? "Preparando..." : "Ver acceso PagaCuotas"}
      </button>
