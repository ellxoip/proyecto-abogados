...
export default async function LibroComprasPage({ searchParams }: { searchParams: { mes?: string } }) {
  const hoy = new Date();
  ...
  const documentos = await prisma.documentoCompra.findMany({
    where: { fecha_emision: { gte: inicio, lte: fin }, estado: { not: "ANULADO" } },
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <div className="card p-4">
          <p className="text-xs text-[var(--muted)]">IVA crédito fiscal</p>
          <p className="mt-1 text-xl font-bold text-blue-600">{formatCurrency(totales.iva)}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-[var(--muted)]">Total compras</p>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-xs text-[var(--muted)]">
            <tr>
              ...
              <th className="table-cell text-left font-medium">Razón social emisor</th>
              <th className="table-cell text-left font-medium">RUT emisor</th>
            ...
            {documentos.map(d => {
              ...
              return (
                <tr key={d.id} className="hover:bg-slate-50">
                  ...
                  <td className="table-cell text-[var(--muted)]">{d.proveedor.rut}</td>
