...
export default async function ReporteVentasDocumentosPage({ searchParams }: { searchParams: { desde?: string; hasta?: string; tipo?: string; estado?: string } }) {
  const hoy = new Date();
  ...
  return (
    <section className="space-y-6">
      <header>
        ...
        <h2 className="mt-1 text-2xl font-semibold">Documentos emitidos</h2>
        <p className="text-sm text-[var(--muted)]">Boletas, facturas y notas de crédito del período</p>
      </header>
      ...
      <form method="get" className="card p-4 grid gap-4 sm:grid-cols-5">
        ...
        <div>
          ...
          <select name="tipo" defaultValue={searchParams.tipo ?? ""}
            ...
            {["BOLETA","FACTURA_EXENTA","FACTURA_AFECTA","NOTA_CREDITO","COMPROBANTE_INGRESO"].map(t => (
        ...
        <div>
          ...
          <select name="estado" defaultValue={searchParams.estado ?? ""}
            className="w-full rounded-md border border-[var(--border)] px-3 py-2 text-sm">
            <option value="">Todos</option>
            {["EMITIDO","PAGADO","ANULADO","ACEPTADO_SII"].map(e => (
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="card p-4">
          <p className="text-xs text-[var(--muted)]">IVA</p>
          <p className="mt-1 text-xl font-bold">{formatCurrency(totales.iva)}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-[var(--muted)]">Total ({documentos.length} documentos)</p>
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {documentos.map(d => (
              <tr key={d.id} className="hover:bg-slate-50">
                <td className="table-cell text-[var(--muted)]">{formatDate(d.fecha_emision)}</td>
                <td className="table-cell text-xs">{d.tipo.replace(/_/g, " ")}</td>
                <td className="table-cell text-[var(--muted)]">{d.numero ?? "—"}</td>
                <td className="table-cell">{d.cliente?.nombre ?? d.razon_social}</td>
                <td className="table-cell text-right">{formatCurrency(Number(d.monto_neto))}</td>
                <td className="table-cell text-right text-[var(--muted)]">{formatCurrency(Number(d.iva))}</td>
                <td className="table-cell text-right font-semibold">{formatCurrency(Number(d.monto_total))}</td>
                <td className="table-cell">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${d.estado === "PAGADO" ? "bg-emerald-50 text-emerald-700" : d.estado === "ANULADO" ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-700"}`}>
                    {d.estado}
                  </span>
                </td>
              </tr>
