...
const TIPOS_FACTURA = ["FACTURA_AFECTA", "FACTURA_EXENTA"] as const;
type TipoFactura = (typeof TIPOS_FACTURA)[number];
...
export default function FacturasPage() {
  ...
  const [tipo, setTipo] = useState<TipoFactura>("FACTURA_EXENTA");
  const [estado, setEstado] = useState("");

  async function load(t: string, e: string) {
  ...
  const totales = docs.reduce(
    (a, d) => ({
      neto: a.neto + Number(d.monto_neto),
      iva: a.iva + Number(d.iva),
      total: a.total + Number(d.monto_total),
    }),
    ...
    <section className="space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-semibold">Facturas</h2>
          <p className="text-sm text-[var(--muted)]">Facturas afectas y exentas emitidas</p>
        </div>
        <Link
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-4 text-sm">
          ...
          <span className="text-[var(--muted)]">
            IVA: <strong>{formatCurrency(totales.iva)}</strong>
          </span>
          <span className="text-[var(--muted)]">
