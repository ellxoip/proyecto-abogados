...
export class AtInformaClient {
  ...
  constructor(options: AtInformaClientOptions = {}) {
    this.baseUrl =
      ...
      process.env.HIVE_SERVICE_URL ??
      "";
    this.token =
      options.token ??
      process.env.AT_INFORMA_TOKEN ??
      process.env.HIVE_SERVICE_API_KEY ??
      "";
    this.fetchFn = options.fetchFn ?? fetch;

    if (!this.baseUrl) {
      throw new Error("AT_INFORMA_BASE_URL/HIVE_SERVICE_URL no est\u00e1 definido.");
    }
    if (!this.token) {
      throw new Error("AT_INFORMA_TOKEN/HIVE_SERVICE_API_KEY no est\u00e1 definido.");
    }
  }
  ...
  async syncPaymentLink(payload: Record<string, unknown>) {
    return this.request<{ ok: boolean; clientId?: string; email?: string }>(
      "/api/internal/integration/clients/payment-link",
      {
        method: "POST",
        body: JSON.stringify(payload),
      },
