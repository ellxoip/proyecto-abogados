...
export class PagaCuotasIntegrationService {
  ...
  private async triggerInitialPaymentActions(
    ...
    let casePayload: Record<string, unknown> | null = null;
    try {
      const portalAccess = await this.getPagaCuotasAccessSnapshot(contrato.id);
      if (!portalAccess?.passwordPlain || !portalAccess.paymentLink) {
        throw new Error(
          `No hay link/clave PagaCuotas procesados para contrato ${contrato.id}. No se puede crear caso en Hive Service.`,
        );
      }

      const caseCode = `SIS-${contrato.id}`;
      casePayload = {
        rut: cliente.rut,
        nombre: cliente.nombre,
        email: cliente.email ?? null,
        telefono: cliente.telefono ?? null,
        password_plain: portalAccess.passwordPlain,
        case_code: caseCode,
        ...
        payment_link: portalAccess.paymentLink,
  ...
  private async getPagaCuotasAccessSnapshot(contratoId: number) {
    const event = await this.db.integrationEvent.findUnique({
      where: { idempotency_key: `pagacuotas:client:from-crm:contrato=${contratoId}` },
    });
    if (!event || event.status !== IntegrationEventStatus.PROCESSED) return null;

    const result = (event.result_payload ?? {}) as {
      passwordPlain?: string | null;
      paymentLink?: string | null;
    };
    return {
      passwordPlain: result.passwordPlain ?? null,
