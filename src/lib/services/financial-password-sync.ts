...
export async function syncClientPasswordToFinancial(payload: SyncPayload): Promise<void> {
  ...
  try {
    // PATCH idempotente (fija newPassword) → reintentar es seguro. Timeout
    // corto + 2 intentos para no colgar el cambio de clave del cliente si fc
    // no responde; sigue siendo fail-soft (un job posterior reconcilia).
